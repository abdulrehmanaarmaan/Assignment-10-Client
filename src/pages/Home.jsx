...
const Home = () => {
    ...
    // ================= MOVIE DERIVED DATA (optimized) =================
    const sortedMoviesByRating = useMemo(
        () => [...allMovies].sort((a, b) => b.rating - a.rating),
        [allMovies]
    );

    const fiveMostRatedMovies = useMemo(
        () => sortedMoviesByRating.slice(0, 5),
        [sortedMoviesByRating]
    );

    const threeMostRatedMovies = useMemo(
        () => sortedMoviesByRating.slice(0, 3),
        [sortedMoviesByRating]
    );

    const lastSixMovies = useMemo(
        () => [...allMovies].reverse().slice(0, 6),
        [allMovies]
    );
    ...
    return (
        <div className='space-y-24 px-4'>
            ...
            {/* ================= FEATURED ================= */}
            <section>
                <h1 className='font-bold text-4xl md:text-6xl text-center mb-10 animate__animated animate__fadeInUp'>
                    Featured Movies
                </h1>

                <FeaturedMovie
                    threeMostRatedMovies={threeMostRatedMovies}
                    loading={loading}
                />
            </section>
            ...
            <section className='mt-32'>
                <h1 className='text-3xl font-semibold text-gray-800 tracking-tight mb-4 text-center route-title'>
                    Statistics
                </h1>
                ...
                <section className='flex md:justify-center justify-between items-center md:gap-[200px] gap-4 font-bold text-center px-4 md:flex-row flex-col'>
                    ...
                    <div className='bg-base-100 rounded-xl p-6 border border-gray-300 form'>
                        ...
                        {loading ? <Loader /> : <h1 className='text-2xl font-bold'>{allMovies.length}</h1>}
                    </div>

                    <div className='bg-base-100 rounded-xl p-6 border border-gray-300 form'>
                        <h1 className='text-gray-500 web-data'>Total Users</h1>
                        {loading ? <Loader /> : <h1 className='text-2xl font-bold'>{users.length}</h1>}
                    </div>
                ...
                <section className='flex flex-wrap gap-10 text-left justify-center px-4'>
                    {loading
                        ...
                        : genres.map((genre, index) => (
                            <span
                                key={index}
                                className='font-medium text-sm px-4 py-2 rounded-full bg-gray-100 text-gray-700 .web-app genre-bg'
                            >
                                {genre}
                            </span>
                        ))
            ...
            <section>
                ...
                <div className='max-w-5xl mx-auto text-lg space-y-6 leading-relaxed'>
                    <p className='leading-relaxed'>
                        <span className='font-bold text-red-600 platform'>MovieMaster Pro</span> is a next generation movie management and discovery platform built for true film lovers.
                    ...
                    <ul className='list-disc list-outside pl-5 space-y-3 md:space-y-4'>
                        <li className='leading-relaxed'>
                            <span className='font-bold text-green-700 feature'>Add & Manage:</span> Add your favorite movies and edit details easily.
                        </li>
                        <li className='leading-relaxed'>
                            <span className='font-bold text-green-700 feature'>Rating Filter:</span> Filter by ratings using dynamic range filters with real-time updates.
                        </li>
                        <li className='leading-relaxed'>
                    ...
                    <p className='leading-relaxed'>
                        With <span className='font-bold text-red-600 platform'>MovieMaster Pro</span>, experience movies like never before - your personal cinema dashboard powered by modern technology.
                    </p>
                </div>
            </section>
